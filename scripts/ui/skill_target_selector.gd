...
## 技能目标选择器 - 纯逻辑类，直接操作HUD
## 支持缩放、平移、比例尺
## 不继承Node，用辅助Node接收_input事件

class_name SkillTargetSelector
extends RefCounted
...
var _grid_container: Node = null
var _self_marker: ColorRect = null
var _scale_label: Label = null
var _zoom_label: Label = null

func _init(hud: CanvasLayer, vehicle: Node, skill_id: String) -> void:
	_hud = hud
	_vehicle = vehicle
	_skill_id = skill_id
	print("[SkillTargetSelector] 初始化, hud=%s" % hud.name)
	# 优先从Main节点的map_config读取（实际加载的地图配置，最可靠）
	var main_scene = _hud.get_tree().current_scene
	if main_scene and "map_config" in main_scene:
		var cfg = main_scene.map_config
		if cfg and cfg.has("size"):
			_map_size = float(cfg["size"])
	# 回退：从DataLoader读取
	if _map_size <= 0 and Engine.has_singleton("DataLoader"):
		var map_cfg = DataLoader.get_map(GameManager.selected_map_id)
		if map_cfg and map_cfg.has("size"):
			_map_size = float(map_cfg["size"])
	print("[SkillTargetSelector] 地图大小: %.0fm, map_id=%s" % [_map_size, GameManager.selected_map_id])
	# 初始视图中心 = 地图中心(0,0)，确保地图边界线在显示区域边缘
	_view_center = Vector2.ZERO
	_build_ui()
	_start_mouse_timer()
	_start_input_helper()
	Input.set_mouse_mode(Input.MOUSE_MODE_VISIBLE)
	print("[SkillTargetSelector] UI构建完成, 地图%.0fm" % _map_size)

func _start_input_helper() -> void:
	"""创建辅助Node接收_input事件（RefCounted不能直接接收_input）"""
	_input_helper = Node.new()
	_input_helper.name = "SkillTargetInputHelper"
	_input_helper.set_script(load("res://scripts/ui/_target_input_helper.gd"))
	_input_helper.set_meta("selector", self)
	_hud.add_child(_input_helper)
	_ui_nodes.append(_input_helper)
...
func _start_mouse_timer() -> void:
	...
	_hud.add_child(_mouse_timer)
	_ui_nodes.append(_mouse_timer)
...
func _build_ui() -> void:
	var vp_size = _hud.get_viewport().get_visible_rect().size
	...
	_ui_nodes.append(title)

	# 提示
	var hint = Label.new()
	hint.text = "左键选择目标 | 滚轮缩放 | 右键/中键拖动平移 | 重复技能键取消"
	...
	_hud.add_child(map_bg)
	_ui_nodes.append(map_bg)

	# 网格线容器
	_grid_container = Node.new()
	_grid_container.name = "GridContainer"
	_hud.add_child(_grid_container)
	_ui_nodes.append(_grid_container)
	_rebuild_grid()
	...
	if _vehicle and is_instance_valid(_vehicle):
		...
		_hud.add_child(_self_marker)
		_ui_nodes.append(_self_marker)
	...
	_hud.add_child(_marker)
	_ui_nodes.append(_marker)

	# 点击捕获层（放在所有按钮/标签之前，这样按钮在上面可正常点击）
	_click_catcher = Control.new()
	_click_catcher.position = Vector2.ZERO
	_click_catcher.size = vp_size
	_click_catcher.mouse_filter = Control.MOUSE_FILTER_STOP
	_click_catcher.gui_input.connect(_on_gui_input)
	_hud.add_child(_click_catcher)
	_ui_nodes.append(_click_catcher)

	# 比例尺（左下角）
	_scale_label = Label.new()
	_scale_label.position = Vector2(_map_rect.position.x + 10, _map_rect.position.y + _map_rect.size.y - 28)
	_scale_label.size = Vector2(220, 22)
	...
	_hud.add_child(_scale_label)
	_ui_nodes.append(_scale_label)
	_update_scale_label()

	# 缩放级别（右上角）
	_zoom_label = Label.new()
	_zoom_label.position = Vector2(_map_rect.position.x + _map_rect.size.x - 130, _map_rect.position.y + 6)
	_zoom_label.size = Vector2(120, 22)
	...
	_hud.add_child(_zoom_label)
	_ui_nodes.append(_zoom_label)
	_update_zoom_label()

	# 缩放按钮（右下角，带背景面板更明显）
	var zoom_panel = Panel.new()
	zoom_panel.position = Vector2(_map_rect.position.x + _map_rect.size.x - 118, _map_rect.position.y + _map_rect.size.y - 44)
	zoom_panel.size = Vector2(110, 40)
	zoom_panel.modulate = Color(0.85, 0.88, 0.82, 0.95)
	_hud.add_child(zoom_panel)
	_ui_nodes.append(zoom_panel)

	var zoom_btn_row = HBoxContainer.new()
	zoom_btn_row.position = Vector2(5, 4)
	...
	_hud.add_child(btn_row)
	_ui_nodes.append(btn_row)
...
func _refresh_view() -> void:
	...
	if _self_marker and _vehicle and is_instance_valid(_vehicle):
		var vpos = _vehicle.global_position
		var self_screen = _world_to_map(vpos.x, vpos.z)
		# 如果自己在可见区域外，固定在地图边缘并变色指示
		var map_rect = Rect2(_map_rect.position.x + 5, _map_rect.position.y + 5, _map_rect.size.x - 10, _map_rect.size.y - 10)
		if not map_rect.has_point(self_screen):
			# 夹在地图边缘
			var clamped = Vector2(
				clamp(self_screen.x, map_rect.position.x, map_rect.position.x + map_rect.size.x),
				clamp(self_screen.y, map_rect.position.y, map_rect.position.y + map_rect.size.y)
			)
			_self_marker.position = clamped - Vector2(5, 5)
			_self_marker.size = Vector2(10, 10)
			_self_marker.color = Color(1, 0.9, 0.2, 1)  # 黄色表示在视野外
		else:
			_self_marker.position = self_screen - Vector2(7, 7)
			_self_marker.size = Vector2(14, 14)
			_self_marker.color = Color(0.3, 1, 0.4, 1)  # 绿色表示在视野内
		_self_marker.visible = true
